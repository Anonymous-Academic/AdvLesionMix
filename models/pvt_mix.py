...
class Features(nn.Module):
    def __init__(self, net_layers):
        ...
        net_layers_ = list(net_layers[1].children())
        
        self.net_layer_0 = nn.Sequential(net_layers[0])
        
        self.net_layer_1 = nn.Sequential(net_layers_[0])
        self.net_layer_2 = nn.Sequential(net_layers_[1])
        self.net_layer_3 = nn.Sequential(net_layers_[2])
        self.net_layer_4 = nn.Sequential(net_layers_[3])
    ...
    def forward(self, x):
        ...
        x1 = self.net_layer_2(x)

        x2 = self.net_layer_3(x1)
        x3 = self.net_layer_4(x2)
                
        return x1, x2, x3
    
class Network_Wrapper(nn.Module):
    def __init__(self, net_layers, num_class, kernels, dims, inner_dim):
        super().__init__()
...
def swin_base(num_class, inner_dim=256, generator_wrapper="Generator_Wrapper"):
...
def pvt_mix(num_class, inner_dim=256, generator_wrapper="Generator_Wrapper"):
    ...
    import timm
    net = timm.create_model("pvt_v2_b0.in1k", pretrained=True)
    ...
    return Network_Wrapper(net_layers, num_class, kernels, dims, inner_dim), \
                This_Generator(dims, num_g_layers),\
                    Discriminator(dims, inner_dim)
